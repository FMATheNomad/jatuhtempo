...
import {
  ...
  Users, Wallet, Check, Lightbulb, MessageCircle, Globe,
  AlertTriangle
} from 'lucide-react'
...
function LandingPage() {
  ...
  const statItems = [
    { icon: Smartphone, val: '1K+', label: t('Pengguna Aktif', 'Active Users') },
    { icon: Wallet, val: 'Rp10M+', label: t('Utang Tercatat', 'Debts Tracked') },
    { icon: CheckCircle2, val: '100+', label: t('Utang Lunas', 'Debts Paid') },
  ]
      ...
      <nav className="sticky top-0 z-50 border-b border-slate-200 dark:border-white/10 bg-white/80 dark:bg-[#0a0b1e]/80 backdrop-blur-md">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
          <div className="flex items-center gap-3">
            <img src="/assets/logo-icon.png" alt="JatuhTempo" className="h-9 w-auto" />
      ...
      <section className="mx-auto max-w-4xl px-4 pb-20">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {statItems.map((item, i) => {
            ...
            return (
            <div key={item.label} className="bg-white dark:bg-white/5 rounded-xl p-4 border border-slate-200 dark:border-white/10 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-white/60">{item.label}</p>
              </div>
            </div>
            )
          })}
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-3xl mx-auto">
            ...
            <div className="rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 p-8 text-center">
              ...
              <ul className="space-y-3 text-sm text-left mb-8">
                {[
                  [t('Catat utang tanpa batas', 'Unlimited debt tracking'), true],
                  [t('Foto tagihan 5x/bulan', 'Photo scan 5x/month'), true],
                  [t('Reminder Telegram', 'Telegram reminders'), true],
                  [t('Dashboard & Strategi', 'Dashboard & Strategy'), true],
                  [t('Export CSV/PDF', 'Export CSV/PDF'), false],
                  [t('Debt Health Score', 'Debt Health Score'), false],
                ].map(([label, included]) => (
                  <li key={typeof label === 'string' ? label : ''} className="flex items-center gap-3">
                    {included ? <Check className="w-4 h-4 text-emerald-500 shrink-0" /> : <X className="w-4 h-4 text-slate-300 dark:text-slate-600 shrink-0" />}
                    <span className={included ? 'text-slate-600 dark:text-slate-300' : 'text-slate-400 dark:text-slate-500'}>{label}</span>
                  </li>
                ))}
              </ul>
            ...
            <div className="rounded-2xl border-2 border-teal-500 bg-white dark:bg-white/5 p-8 text-center relative">
              ...
              <ul className="space-y-3 text-sm text-left mb-8">
                {[
                  [t('Catat utang tanpa batas', 'Unlimited debt tracking'), true],
                  [t('Foto tagihan tanpa batas', 'Unlimited photo scan'), true],
                  [t('Reminder prioritas', 'Priority reminders'), true],
                  [t('Dashboard & Strategi', 'Dashboard & Strategy'), true],
                  [t('Export CSV/PDF', 'Export CSV/PDF'), true],
                  [t('Debt Health Score', 'Debt Health Score'), true],
                ].map(([label, included]) => (
                  <li key={typeof label === 'string' ? label : ''} className="flex items-center gap-3">
                    {included ? <Check className="w-4 h-4 text-emerald-500 shrink-0" /> : <X className="w-4 h-4 text-slate-300 dark:text-slate-600 shrink-0" />}
      ...
      <form onSubmit={handleAdd} className={className}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <select value={form.interest_type} onChange={e => setForm({...form, interest_type: e.target.value})} className={isHero ? "h-12 rounded-xl bg-white/10 border border-white/20 text-white px-4 text-sm" : "h-12 rounded-xl border border-input bg-background px-4 text-sm"}>
            ...
            {rateSuggestion && (
              <div className="w-full text-xs text-accent font-medium flex items-center gap-1">
                <Lightbulb className="w-3.5 h-3.5" /> Saran: bunga {rateSuggestion.rate}%/{rateSuggestion.type === 'monthly' ? 'bln' : rateSuggestion.type === 'daily' ? 'hari' : rateSuggestion.type === 'yearly' ? 'thn' : rateSuggestion.type}
              </div>
            )}
            ...
            <div className="bg-gradient-to-br from-primary to-[#1a1a4e] rounded-2xl p-8 text-white text-center">
              ...
              <div className="flex items-center justify-center mb-6">
                <a href="https://t.me/JatuhTempo_bot" target="_blank" className="inline-flex items-center gap-2 text-xs text-white/70 hover:text-white bg-white/10 hover:bg-white/20 px-4 py-2 rounded-full transition-all">
                  <MessageCircle className="w-4 h-4" /> Buka Bot Telegram
                </a>
              </div>
          ...
          {summary && !empty && (
            <>
              ...
              <Card>
                ...
                {rateSuggestion && (
                  <div className="text-xs text-accent font-medium bg-accent/5 border border-accent/20 rounded-lg px-3 py-2 flex items-center gap-1.5">
                  ...
                  <div className="space-y-3">
                    {debts.slice(0, 5).map((d: any) => (
                      <div key={d.id} className="flex items-center justify-between p-3 rounded-lg hover:bg-secondary/50 transition-colors">
                        <div className="flex items-center gap-3">
                          <Badge variant={statusVariant[d.status] || 'default'}>{d.status === 'active' ? 'Aktif' : d.status === 'paid' ? 'Lunas' : 'Terlambat'}</Badge>
                          <div>
